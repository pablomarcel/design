    ...
    def solve_flags(
        ...
        *,
        ...
        if solve_path in {'general_3d_stress', 'plane_stress_rotation'}:
            ...
            inputs = StressTensorInput(sxx=float(sxx), syy=float(syy), szz=float(szz), txy=float(txy), tyz=float(tyz), txz=float(txz), phi_deg=phi_deg, unit=unit, title=title)
            ...
            inputs = StrainTensorInput(exx=float(exx), eyy=float(eyy), ezz=float(ezz), gxy=float(gxy), gyz=float(gyz), gxz=float(gxz), phi_deg=phi_deg, unit=unit, title=title)
            ...
            inputs = RosetteInput(ea=float(ea), eb=float(eb), ec=float(ec), theta_a_deg=float(theta_a_deg), theta_b_deg=float(theta_b_deg), theta_c_deg=float(theta_c_deg), unit=unit, title=title, E=E, nu=nu, G=G, stress_unit=stress_unit, phi_deg=phi_deg)
            ...
            inputs = Hooke3DFromStrainInput(exx=float(exx), eyy=float(eyy), ezz=float(ezz), gxy=float(gxy), gyz=float(gyz), gxz=float(gxz), E=float(E), nu=nu, G=G, unit=unit, stress_unit=stress_unit, title=title)
            ...
            if missing:
                raise ValueError(f"Missing required flags for {solve_path}: {', '.join(missing)}")
            inputs = SingleGaugePlaneStressInput(epsilon_theta=float(epsilon_theta), theta_deg=float(theta_deg), sigma_x_known=float(known_sigma_x), E=float(E), nu=nu, G=G, unit=unit, stress_unit=stress_unit, title=title, phi_deg=phi_deg)
        else:
